...
def print_function_entry_and_exit(decorated_function):
    ...
    def wrapper(*dec_fn_args, **dec_fn_kwargs):
        ...
        func_args = (
            ...
            .bind(*dec_fn_args, **dec_fn_kwargs)
            .arguments
        )
        recursion_depth = 0
        ...
        func_args_str = ", ".join(
            f"{var_name} = {var_value}" for var_name, var_value in func_args.items()
        )
...
def pretty_print_list(my_list, sep=", ", and_char=", & ", binary_op="and"):
    return (
        ...
        if len(my_list) > 2
        else "{} {} {}".format(my_list[0], binary_op, my_list[1])
        if len(my_list) == 2
        else my_list[0]
    )
...
class TextSearchTypes(Enum):
    ...
    def get_search_query_suffix(cls, search_type):
        query_suffix = (
            "iregex"
            if search_type == TextSearchTypes.CASE_INSENSITIVE_REGEX
            else "regex"
            if search_type == TextSearchTypes.CASE_SENSITIVE_REGEX
            else "contains"
            if search_type == TextSearchTypes.CASE_SENSITIVE_CONTAINS
            else "icontains"
            if search_type == TextSearchTypes.CASE_INSENSITIVE_CONTAINS
            else "search"
        )
        return query_suffix
...
def GetSearchQuerySuffix(search_type):
    query_suffix = (
        ...
        if search_type == TextSearchTypes.ENDSWITH_SENSITIVE
        else "iendswith"
        if search_type == TextSearchTypes.ENDSWITH_INSENSITIVE
        else "search"
    )
    return query_suffix
...
class ConceptQuerySet(CommonSNOMEDQuerySet):
    ...
    def by_fully_specified_names(self, **kwargs):
        return self.filter(
            id__in=Description.fully_specified_names()
            .filter(**kwargs)
            .values_list("concept__id", flat=True)
        )

    @property
    def fully_specified_names(self):
        return (
            Description.fully_specified_names()
            .filter(concept__in=self)
    ...
    def fully_specified_names_no_type(self):
        return [
            SNOMED_NAME_PATTERN.search(term).group("name")
            for term in Description.fully_specified_names()
            .filter(active=True, term__isnull=False, concept__in=self)
            .values_list("term", flat=True)
        ]

    async def afully_specified_names_no_type(self):
        qs = (
            Description.fully_specified_names()
            .filter(active=True, term__isnull=False, concept__in=self)
            .values_list("term", flat=True)
        ...
        return [
            SNOMED_NAME_PATTERN.search(term).group("name")
            async for term in qs.aiterator()
        ]
...
class ConceptManager(SNOMEDCTModelManager):
    ...
    def by_full_specified_names(
        self, search_strings, search_type=TextSearchTypes.CASE_INSENSITIVE_CONTAINS
    ):
        ...
        query = reduce(
            ...
            (
                Q(
                    ...
                    **{"term__{}".format(query_suffix): item},
                )
                for item in search_strings
            ),
        )
        return self.get_queryset().filter(
            id__in=Description.objects.filter(query).values_list(
                "concept_id", flat=True
            )
        )

    def by_fully_specified_name(
        self, search_string, search_type=TextSearchTypes.CASE_INSENSITIVE_CONTAINS
    ):
        query_suffix = GetSearchQuerySuffix(search_type)
        kwargs = {
            "term__{}".format(query_suffix): search_string,
            "type_id": DESCRIPTION_TYPES["Fully specified name"],
        }
    ...
    def mapped(self):
        return self.get_queryset().filter(
            pk__in=ICD10_Mapping.objects.all().values_list(
                "referenced_component__pk", flat=True
            )
        )
...
DEFINITION_STATUS_MAPPING = {"Primitive": PRIMITIVE_CONCEPT, "Defined": DEFINED_CONCEPT}
...
class Concept(CommonSNOMEDModel):
    ...
    def expository_text(self):
        ...
        classification_names_w_article = [
            "a/an '{}'".format(c.fully_specified_name_no_type) for c in self.isa
        ]
        non_isa_relationships = self.outbound_relationships().exclude(type_id=ISA)
        roleGroupTextParts = [
            "{} '{}'".format(
                ATTRIBUTE_HUMAN_READABLE_NAMES.get(
                    rel.type.id, rel.type.fully_specified_name_no_type
                ),
        ...
        if is_primitive:
            ...
            definition_head = pretty_print_list(
                classification_names_w_article, and_char=", and "
            )
            definition_text = "({}) that {}".format(
                definition_head,
                pretty_print_list(roleGroupTextParts, and_char=", and "),
            )
        return "Every '{}' is {}".format(
            self.fully_specified_name_no_type, definition_text
        )

    @property
    def concepts(self):
        return SNOMED_NAME_PATTERN.search(self.get_fully_specified_name().term).group(
            "name"
        )
    ...
    @classmethod
    def by_fully_specified_name_and_mapped(
        cls, search_string, search_type=TextSearchTypes.CASE_INSENSITIVE_CONTAINS
    ):
        query_suffix = GetSearchQuerySuffix(search_type)
        kwargs = {
            "term__{}".format(query_suffix): search_string,
        ...
        return cls.objects.mapped().filter(
            id__in=Description.objects.filter(**kwargs).values_list(
                "concept_id", flat=True
            )
        )
    ...
    def get_fully_specified_name(self, lang="en_us"):
        return cache.get_or_set(
            "fsn_%d" % self.id, lambda: self.__get_fully_specified_name(lang), None
        )

    def get_preferred_term(self, lang="en_us"):
        return cache.get_or_set(
            "pt_%d" % self.id, lambda: self.__get_preferred_term(lang), None
        )
    ...
    def related_concepts(self, **kwargs):
        return Concept.objects.filter(
            id__in=self.outbound_relationships()
            .filter(**kwargs)
    ...
    def part_of_transitive(
        self, chain=None, stop_concepts=None, recursion_depth=0, **kwargs
    ):
        stop_concepts = (
            stop_concepts
            if stop_concepts
    ...
    def specializations(self):
        return Concept.objects.filter(
            id__in=self.inbound_relationships()
            .filter(type_id=ISA)
...
class TransitiveClosure(models.Model):
    ...
    start = models.ForeignKey(
        Concept, on_delete=models.PROTECT, related_name="transitive_subsumes"
    )
    end = models.ForeignKey(
        Concept, on_delete=models.PROTECT, related_name="transitive_isa"
    )
...
class Description(CommonSNOMEDModel):
    TYPE_CHOICES = ((code, label) for label, code in DESCRIPTION_TYPES.items())
...
class TextDefinition(CommonSNOMEDModel):
    ...
    type = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        choices=Description.TYPE_CHOICES,
        db_index=False,
        db_column="typeId",
        related_name="+",
    )
...
class RelationshipQuerySet(CommonSNOMEDQuerySet):
    ...
    def destinations(self):
        return Concept.objects.filter(
            id__in=self.values_list("destination_id", flat="True")
        )
    ...
    def unique_links(self):
        ...
        for rel_id, src_id, type_id, dest_id in self.values_list(
            "id", "source_id", "type_id", "destination_id"
        ):
...
class Relationship(CommonSNOMEDModel):
    ...
    modifier = models.ForeignKey(
        Concept, on_delete=models.PROTECT, related_name="+", db_column="modifierId"
    )
...
class LangRefSet(CommonSNOMEDModel):
    ...
    refset = models.ForeignKey(
        Concept, on_delete=models.PROTECT, choices=REFSET_CHOICES, related_name="+"
    )
    referenced_component = models.ForeignKey(
        Description, on_delete=models.PROTECT, related_name="lang_refset"
    )
    acceptability = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
...
class AssociationRefSet(CommonSNOMEDModel):
    ...
    referenced_component = models.ForeignKey(
        Concept, on_delete=models.PROTECT, related_name="+"
    )
...
class ExtendedMapRefSet(CommonSNOMEDModel):
    ...
    refset = models.ForeignKey(
        Concept, on_delete=models.PROTECT, related_name="+", db_column="refsetId"
    )
    referenced_component = models.ForeignKey(
        Concept,
...
class ICD10_MappingQuerySet(CommonSNOMEDQuerySet):
    ...
    def concepts(self):
        return Concept.objects.filter(
            id__in=self.values_list("referenced_component", flat=True)
        )
...
class ICD10_MappingManager(models.Manager):
    ...
    def by_icd_names(
        ...
        query = reduce(
            or_,
            (
                Q(**{"map_target_name__{}".format(query_suffix): item})
                for item in search_strings
            ),
        )
    ...
    def by_fully_specified_name(
        ...
        return self.get_queryset().filter(
            referenced_component__in=Concept.by_fully_specified_name(
    ...
    def by_icd_codes(self, codes):
        ...
        query = reduce(
            or_, (Q(map_target__startswith=code, map_rule="TRUE") for code in codes)
        )
        return self.get_queryset().filter(query)

    async def aby_icd_codes(self, codes):
        query = reduce(
            or_, (Q(map_target__startswith=code, map_rule="TRUE") for code in codes)
        )
        return self.get_queryset().filter(query)
...
class ICD10_Mapping(ExtendedMapRefSet):
    ...
    referenced_component = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        related_name="icd10_mappings",
        db_column="referencedComponentId",
    )
    referenced_component_name = models.TextField(
        default="", db_column="referencedComponentName"
    )
    map_target_name = models.TextField(
        default="", db_index=True, db_column="mapTargetName"
    )
...
class TermBasedView(BaseSNOMEDCTModel):
    lang_refset_refset = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        choices=LangRefSet.REFSET_CHOICES,
        related_name="+",
        primary_key=True,
    )
    lang_refset_acceptability = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        choices=LangRefSet.ACCEPTABILITY_CHOICES,
        related_name="+",
    )
    description = models.ForeignKey(
        Description, on_delete=models.PROTECT, related_name="+"
    )
    ...
    description_case_significance = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        choices=Description.CASE_SIGNIFICANCE_CHOICES,
        related_name="+",
    ...
    concpet_definition_status = models.ForeignKey(
        Concept,
        on_delete=models.PROTECT,
        choices=Concept.DEFINITION_STATUS_CHOICES,
        related_name="+",
    )
    ...
    def __str__(self):
        return "%s: %s" % (
            self.description_language_code.upper(),
